...
class ClusteredATEBaselines:
    ...
    **NEW**: Log verbose output to text files for analysis
    ...
    def __init__(self, X, T, Y, model_type='linear', lambda_reg=0.1, max_iter=1000, find_confounders=False, log_dir='logs'):
        # Store the dataset
    ...
    def top_k(self, target_ate=None, epsilon=0.01, k=None, approx=False, 
        ...
        if use_clustering:
            ...
            for i, rep_idx in enumerate(representative_indices):
                ate_diff = self.ate_model.get_ate_difference([rep_idx], approx=approx, update=False) if self.model_type == 'linear' else \
                            self.ate_model.get_ate_difference([rep_idx], approx=approx, update=False)
                ...
                if need_recalc:
                    ...
                    for cluster_id in clusters:
                        ...
                        for sample_idx in sampled_points:
                            ate_diff = self.ate_model.get_ate_difference([sample_idx], approx=approx, update=False) if self.model_type == 'linear' else \
                                        self.ate_model.get_ate_difference([sample_idx], approx=approx, update=False)
                ...
                if len(available_in_best_cluster) == 1:
                    ...
                    if need_recalc or cluster_cache_key not in cached_point_scores:
                        ...
                        for candidate_idx in available_in_best_cluster:
                            ate_diff = self.ate_model.get_ate_difference([candidate_idx], approx=approx, update=False) if self.model_type == 'linear' else \
                                        self.ate_model.get_ate_difference([candidate_idx], approx=approx, update=False)
